		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				if ($product_discount_query->num_rows) {
                    if ((float)$product_discount_query->row['price'] && (float)$product_query->row['price'] && $this->config->get('theme_default_product_refresh_price_option_discount')) {
				...
				if ($product_special_query->num_rows) {
                    if ((float)$product_special_query->row['price'] && (float)$product_query->row['price'] &&  $this->config->get('theme_default_product_refresh_price_option_special')) {
