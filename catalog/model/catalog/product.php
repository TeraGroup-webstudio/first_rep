...
class ModelCatalogProduct extends Model {
    ...
    public function getProductStickers($product_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_sticker WHERE product_id = '" . (int)$product_id . "' ORDER BY sort_order ASC");
    ...
    public function getRelatedByCategory($product_id) {
        $limit = ($this->config->get('theme_default_product_for_category_count') !='' || $this->config->get('theme_default_product_for_category_count') != 0 ? $this->config->get('theme_default_product_for_category_count') : 4);
        $product_data = array();
