    ...
    public function index() {
        ...
        foreach ($products as $product) {
            ...
            if ($product['image']  && is_file(DIR_IMAGE . $product['image'])) {
                if ($version < 220) {
                    ...
                    $image_cart_width = $image_cart_width ? $image_cart_width : 74;
                    $image_cart_height = $this->config->get('config_image_cart_height');
                    $image_cart_height = $image_cart_height ? $image_cart_height : 74;
                } elseif ($version < 300) {
        ...
        if ($currentTheme == 'shoppica' || $currentTheme == 'shoppica2') {
            ...
            if (strpos($text_items, '{quantity}') !== false || strpos($text_items, '{total}') !== false) {
                ...
                $this->_templateData['cart_total'] = $this->cart->countProducts();
            } else {
                $this->_templateData['cart_total'] = $this->cart->countProducts();
            } 
        }
